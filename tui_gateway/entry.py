...
def main():
    ...
    # A2A bridge: make this TUI session an A2A peer. Inbound tasks land in
    # the active session (same prompt.submit path the UI uses). Best-effort:
    # no server when the a2a plugin isn't loaded or the port is taken.
    try:
        from tui_gateway.a2a_bridge import start as _start_a2a_bridge

        _start_a2a_bridge()
    except Exception:
        logger.debug("A2A bridge not started", exc_info=True)
